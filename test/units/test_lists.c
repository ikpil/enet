...
TEST(enet_list_insert) {
    ...
    // for begin
    {
        ...
        ASSERT_THAT(((TestNode *)b0)->value, 2);
        ASSERT_THAT(((TestNode *)b1)->value, 1);
        ASSERT_THAT(((TestNode *)b2)->value, 0);
    }
    ...
    // for end
    {
        ...
        ASSERT_THAT(((TestNode *)b0)->value, 0);
        ASSERT_THAT(((TestNode *)b1)->value, 1);
        ASSERT_THAT(((TestNode *)b2)->value, 2);
    }
    ...
    // for position
    {
        ...
        ASSERT_THAT(((TestNode *)b0)->value, 0);
        ASSERT_THAT(((TestNode *)b1)->value, 2);
        ASSERT_THAT(((TestNode *)b2)->value, 1);
    }
    ...
    // Test inserting the same node multiple times
    {
        ...
        // First insert
        enet_list_insert(enet_list_end(&list), &node0);
        ASSERT_THAT(enet_list_size(&list), 1);
...
TEST(enet_list_remove) {
    ...
    // Test 1: Remove already removed node (edge case)
    {
        ...
        ASSERT_THAT(enet_list_size(&list), 0);
    }
    ...
    // Test 2: Remove from beginning
    {
        ...
        ASSERT_THAT(enet_list_size(&list), 2);

        ENetListIterator b0 = enet_list_begin(&list);
        ENetListIterator b1 = enet_list_next(b0);
        ASSERT_THAT(((TestNode *)b0)->value, 1);
        ASSERT_THAT(((TestNode *)b1)->value, 2);
    }
...
TEST(enet_list_move) {
    ...
    ASSERT_THAT(enet_list_size(&list1), 5);
    ASSERT_THAT(enet_list_size(&list2), 5);
    ...
    for (ENetListNode *current = enet_list_begin(&list1);
        ...
        ASSERT_THAT(node->value, expected_values1[i++]);
    }
    ...
    for (ENetListNode *current = enet_list_begin(&list2);
         ...
         current = enet_list_next(current)) {
        TestNode *node = (TestNode *) current;
        ASSERT_THAT(node->value, expected_values2[i++]);
    }
    ...
    // ASSERT_THAT(enet_list_size(&list1), 6);
    // ASSERT_THAT(enet_list_size(&list2), 4);
    //
    // // Test 4: Move multiple nodes at once
    // enet_list_move(enet_list_end(&list2), &nodes[0].node, &nodes[3].node);
    // ASSERT_THAT(enet_list_size(&list1), 3);
    // ASSERT_THAT(enet_list_size(&list2), 7);
    //
...
TEST(enet_list_clear) {
    ...
    ASSERT_THAT(enet_list_size(&list), 3);
